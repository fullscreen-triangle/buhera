...
\title{Resolution of Gibbs' Paradox Through Categorical State Distinguishability and Oscillatory Entropy}
...
Furthermore, if one re-separates the gases after mixing, traditional thermodynamics predicts the system returns to entropy $S_\text{initial}$, suggesting the entire process is reversible. However, this contradicts the second law's requirement that spontaneous processes in isolated systems should increase total entropy.
...
\begin{remark}
The quadratic scaling $\propto N^2$ arises because entropy in our framework is determined by the number of edges (pairwise interactions), not the number of particles. For a fully connected graph: $|E| \propto N^2$.
...
For $\Delta \bar{k} \approx 1$ and $\epsilon \bar{k} \approx 1$: $\Delta S_{\text{phase-lock}} \approx Nk$, consistent with traditional result.
\end{remark}

\subsection{Doesn't Need to Be Complicated}

\begin{remark}
The profound insight is simple:

\textbf{Gas molecules must synchronize their oscillations} (due to Van der Waals, dipoles, collisions).

\textbf{Mixing increases the number of molecules that must synchronize}.

\textbf{More synchronization constraints → more categorical states completed → higher entropy}.
...
\textbf{Every measured value becomes precise "by association"} through the phase-lock network. A molecule's state is not independent but constrained by its graph neighbors. The more neighbors (edges), the tighter the constraints, the more precisely defined the arrangement, the higher the entropy (fewer permissible arrangements).
\end{remark}
