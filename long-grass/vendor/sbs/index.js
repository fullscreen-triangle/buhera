...
// Vendored surface extension (long-grass CKG page): expose the compiler so a
// host can read the raw compiled artifacts — ast, emitted js, glsl, circuit —
// that runSBS() intentionally trims from its serialisable result. The
// "Compiled" tab of the SBS sandbox panel renders exactly these four fields.
export { compileSBS, validateSBS } from './dsl/compiler.js';
export { SCRIPTS, DEFAULT_SCRIPT } from './dsl/scripts/index.js';
// The solver + metrics extractor, so a host that renders its own preview (the
// SBS panel's 3D graph + Charts) can reproduce the sandbox run pipeline exactly
// rather than settle for runSBS()'s trimmed metrics.
export { solveCircuit, computeFluxPattern } from './shaderSolver.js';
export { extractMetrics } from './metricsExtractor.js';
