...
spraypaint verify   [--root DIR] [--json]     # re-check all four invariants; nonzero exit on breach
```
...
```bash
spraypaint index
spraypaint verify --json || { echo "invariant breach"; exit 1; }   # nonzero exit on breach
spraypaint identity --json | jq -r .fingerprint > .spraypaint.fingerprint
```

`verify` re-checks all four invariants and exits nonzero on any breach — a
one-command certificate. Committing the fingerprint lets a later job assert the
index is byte-reproducible for the same content (Inv 1).
