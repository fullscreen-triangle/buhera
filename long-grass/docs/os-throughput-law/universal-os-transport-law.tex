...
\[
\taup^{(ij)}(L) \;=\; \taup^{(ij)}_0 \cdot \exp(\Delta_{ij} L),
\]
where $\taup^{(ij)}_0$ is the unloaded baseline and $\Delta_{ij}$ is a load-sensitivity coefficient. We do not need this dependence for the main theorems but note that it explains the standard observation that kernel throughput degrades exponentially with load \citep{ousterhout1990why,jain1991art}.
...
For a cascade of $k$ stages each with self-coupling $g_i$ and lag $\taup_i$,
\[
\TP^{-1} \;=\; \sum_{i=1}^k \taup_i g_i,
\]
...
\item Cascade: $\TP^{-1} \sim \sum_{i=1}^k \taup_i$ (pipeline serialisation);
...
\[
\frac{\TP_\mathrm{cached}}{\TP_\mathrm{uncached}} \;=\; \frac{\sum_{i,j} \taup^{(ij)} g^{(ij)}}{\taup_\mathrm{lookup}},
\]
