...
\begin{equation}
    \frac{}{⟨n, \rho⟩ \Downarrow n}
    \quad \text{(E-Num)}
\end{equation}

\begin{equation}
    \frac{x \in \text{dom}(\rho)}{⟨x, \rho⟩ \Downarrow \rho(x)}
    \quad \text{(E-Var)}
\end{equation}

\begin{equation}
    \frac{⟨e_1, \rho⟩ \Downarrow v_1 \quad ⟨e_2, \rho⟩ \Downarrow v_2}{⟨e_1 + e_2, \rho⟩ \Downarrow v_1 + v_2}
    \quad \text{(E-Add)}
\end{equation}

\begin{equation}
    \frac{⟨e_1, \rho⟩ \Downarrow v_1 \quad ⟨e_2, \rho⟩ \Downarrow v_2}{⟨e_1 \times e_2, \rho⟩ \Downarrow v_1 \times v_2}
    \quad \text{(E-Mul)}
\end{equation}

\begin{equation}
    \frac{⟨e, \rho⟩ \Downarrow v \quad ⟨n, \rho⟩ \Downarrow k}{⟨e^n, \rho⟩ \Downarrow v^k}
    \quad \text{(E-Pow)}
\end{equation}
...
\begin{equation}
    \frac{⟨s, \rho⟩ \Downarrow \text{stateName} \quad \cataddr = \text{LookupState}(\text{stateName})}{⟨\texttt{resolve final state at } s, \sigma⟩ \Downarrow \sigma[\rho[\text{final\_addr} := \cataddr]]}
\end{equation}

\textbf{Navigate to Penultimate}:
\begin{multline}
    \frac{\text{final\_addr} \in \rho \quad \pentstate = \text{ComputePenultimate}(\rho(\text{final\_addr}))}{⟨\texttt{navigate to penultimate}, \sigma⟩ \Downarrow \sigma[\rho[\text{current} := \pentstate]]}
\end{multline}

\textbf{Complete Trajectory}:
\begin{multline}
    \frac{\text{current} = \pentstate \quad \text{final\_addr} \in \rho \quad \phi = \text{CompletionMorphism}(\pentstate, \rho(\text{final\_addr}))}{⟨\texttt{complete trajectory}, \sigma⟩ \Downarrow \sigma[\rho[\text{current} := \phi(\pentstate)]]}
\end{multline}

\textbf{Memory Create}:
\begin{equation}
    \frac{⟨c, \rho⟩ \Downarrow (s_k, s_t, s_e) \quad \mathcal{M}' = \mathcal{M}[\text{trajectory} \mathrel{+}= (s_k, s_t, s_e)]}{⟨\texttt{memory create at } c, \sigma⟩ \Downarrow \sigma[\mathcal{M} := \mathcal{M}']
\end{equation}

\textbf{Memory Write}:
\begin{multline}
    \frac{⟨v, \rho⟩ \Downarrow \text{val} \quad h = \text{hash}(\mathcal{M}.\text{trajectory}) \quad t = \text{AssignTier}(\|\mathcal{M}.\text{trajectory}\|)}{⟨\texttt{memory write } v \texttt{ at trajectory}, \sigma⟩ \Downarrow \sigma[\mathcal{M}[\text{entries}[h] := (\text{val}, t)]]}
\end{multline}
...
\textbf{Demon Move}:
\begin{multline}
    \frac{⟨c, \rho⟩ \Downarrow (s_k, s_t, s_e) \quad \mathcal{D}' = \mathcal{D}[\text{position} := c, \text{history} \mathrel{+}= c]}{⟨\texttt{demon move to } c, \sigma⟩ \Downarrow \sigma[\mathcal{D} := \mathcal{D}']
\end{multline}

\textbf{Demon Sort} (zero cost):
\begin{equation}
    \frac{\mathcal{D}' = \mathcal{D}[\text{stats.sorts} \mathrel{+}= 1, W_{\text{cat}} := 0]}{⟨\texttt{demon sort by partition}, \sigma⟩ \Downarrow \sigma[\mathcal{D} := \mathcal{D}']
\end{equation}
...
\begin{multline}
    \frac{⟨f, \rho⟩ \Downarrow \text{freq} \quad \omega = 2\pi \cdot \text{freq} \quad \mathcal{C}' = \text{CreateController}(\omega)}{⟨\texttt{controller create at } f, \sigma⟩ \Downarrow \sigma[\mathcal{C} := \mathcal{C}']
\end{multline}

\textbf{Controller Verify}:
\begin{multline}
    \frac{\mathcal{C}.\text{rate} = dM/dt \quad \mathcal{C}.\omega/(2\pi/M) = r_2 \quad 1/⟨\tau_p⟩ = r_3 \quad |dM/dt - r_2| < \epsilon \quad |dM/dt - r_3| < \epsilon}{⟨\texttt{controller verify}, \sigma⟩ \Downarrow \sigma[\omega \mathrel{+}= \text{``VERIFIED''}]
\end{multline}

\subsection{Denotational Semantics}

\begin{definition}[Semantic Domain]
Let $\mathcal{D} = \mathbb{R}^+ \times \text{Dim}$ be the domain of positive reals with dimensions. The semantic function $⟦\cdot⟧$ maps expressions to functions $\text{Env} \to \mathcal{D}$.
\end{definition}

\begin{align}
    ⟦n⟧ \rho &= (n, \mathbf{1}) \\
    ⟦x⟧ \rho &= \rho(x) \\
    ⟦e_1 + e_2⟧ \rho &= \text{add}(⟦e_1⟧ \rho, ⟦e_2⟧ \rho) \\
    ⟦e_1 \times e_2⟧ \rho &= \text{mul}(⟦e_1⟧ \rho, ⟦e_2⟧ \rho)
\end{align}
...
A demon has state:
\begin{equation}
    \mathcal{D} = ⟨\text{position}, \text{history}, \text{aperture}, \text{stats}⟩
\end{equation}
...
\begin{equation}
    \hat{H}\hat{O}_{\text{cat}}|\psi⟩ = \hat{O}_{\text{cat}}\hat{H}|\psi⟩ = E \hat{O}_{\text{cat}}|\psi⟩
\end{equation}
...
A controller has:
\begin{equation}
    \mathcal{C} = ⟨\omega, M, t, \text{history}, \text{verified}⟩
\end{equation}
...
\begin{equation}
    ⟨\tau_p⟩ = \frac{1}{N} \sum_{i=1}^{N} \tau_{p,i}
\end{equation}
\end{definition}
...
\Function{NavigateCached}{addr}
    ...
    \State cache[addr] $\gets$ $⟨$path, now()$⟩$
    \State \Return path
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{enumerate}
    ...
    \item \textbf{Triple Equivalence Verification}: Continuous verification of $dM/dt = \omega/(2\pi/M) = 1/⟨\tau_p⟩$.
