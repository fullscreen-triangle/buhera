...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{Trajectory Completion Mechanism: Framework Synthesis}}
\fancyhead[R]{\small\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\title{\textbf{The Trajectory Completion Mechanism:\\[4pt]
A Comprehensive Synthesis of Categorical Computing across\\[4pt]
Physics, Information, Learning, and Epistemology,\\[4pt]
with Experimental Validation of Every Core Theorem}}
...
\caption{Triple Equivalence validated across 60 $(M,n)$ grid points. (a) Entropy scales as $\kb M \ln n$ across six decades of $n$ and four decades of $M$. (b) Maximum per-point relative error between the three descriptions remains below $10^{-9}$ throughout the grid. (c) 3D surface of $\log_{10} S$ over $(\log_{10} M, \log_{10} n)$. (d) Measured entropy from each description plotted against theory; all three lie on the identity line.}
...
\caption{Processor--Oscillator Duality validated. (a) $S_\mathrm{processor}$ versus $S_\mathrm{oscillator}$ across 35 $(w,n)$ points; all samples lie on the identity line. (b) Processing rate $dM/dt$ versus oscillator frequency for processor widths $M \in \{1, 10, 100, 1000\}$; scaling is strictly proportional. (c) 3D scatter of processor entropy over $(\log_{10} w, \log_{10} n)$. (d) $1/\langle \tau_p \rangle$ versus $M\omega/(2\pi)$; the 16 points fall exactly on the diagonal, confirming the unified equation $dM/dt = M\omega/(2\pi) = 1/\langle \tau_p \rangle$.}
...
\caption{Virtual sub-states and the backward-navigation advantage. (a) 3D scatter of sub-coordinate decompositions $(s_1, s_2, s_3)$, coloured by the global coordinate $s_\mathrm{global}$; points regularly fall outside $[0,1]^3$ yet reconstruct a valid global value, confirming the virtual character of intermediate states. (b) Step counts for backward-with-virtual (teal) versus backward-physical-only (coral) across $N \in \{27, 243, 2187, 19683, 177147\}$: the virtual path tracks $\log_3 N$, the physical-restricted path tracks $\Omega(N)$. (c) Virtual speedup ratio grows from $13.3\times$ at $N=27$ to $24{,}156\times$ at $N=177{,}147$, matching the $(3N-1)/(2\log_3 N)$ prediction. (d) Pass rates for the four virtual-substate theorems: existence, forward feasibility, backward advantage, and path opacity.}
...
\caption{Zero-Cost Sorting Theorem validated. (a) Frobenius commutator norm $\|[\Ocat, \Ophys]\|_F$ scales linearly with the perturbation magnitude $\varepsilon$. (b) Reorder work $W_\mathrm{sort}$ plotted against commutator norm: the two quantities are exactly proportional; at $\varepsilon = 0$ both are zero. (c) 3D surface of $W/(k_BT)$ over $(\varepsilon, \|[\cdot,\cdot]\|_F)$ with the five measurement points overlaid in black. (d) $W/(k_BT)$ at each $\varepsilon$ value; $\varepsilon = 0$ (teal) yields exactly zero thermodynamic cost.}
...
Tested on 25 NIST compounds: the query-coord recovery rate is 25/25 at distance $d = 0.0000$ (exact coord match). Recorded in \texttt{driven/data/integration\_validation\_results.json}.
...
\caption{Penultimate state navigation and the categorical complexity hierarchy. (a) Backward steps from leaf to root at depths 2 through 10; the measurements lie exactly on $y = x$ in $\log_3 N$ coordinates, confirming $\mathcal{O}(\log_3 N)$ complexity. (b) Traversal counts for the five complexity classes across $N \in \{27, 243, 2187, 19683, 177147\}$; strict ordering $C_0 \subsetneq C_1 \subsetneq C_\mathrm{poly} \subsetneq C_\mathrm{nav} \subsetneq C_\mathrm{hard}$ holds at every $N$. (c) 3D bar showing class runtime over $N$; the five classes separate cleanly on the time axis. (d) Traversal ratios $C_\mathrm{hard}/C_1$ and $C_\mathrm{nav}/C_\mathrm{poly}$ track the theoretical $N/\log_3 N$ amplification, showing the hierarchy gap widens with $N$.}
...
\caption{Physical validation on 16 Earth--Moon system properties across ten orders of magnitude. (a) Derived versus observed values on log-log axes; all 16 points lie on the identity diagonal. (b) Relative error per property, sorted; the 5\% threshold is reached only by surface-temperature night where the simple partition model neglects regolith heat conduction. (c) 3D scatter of $\log_{10}$ observed, $\log_{10}$ derived, and relative error (colour). (d) Cumulative distribution of relative errors: 62.5\% of properties are exact, 87.5\% agree to $<1\%$, 94\% agree to $<5\%$.}
